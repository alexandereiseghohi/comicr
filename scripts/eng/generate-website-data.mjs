...
function extractTitle(filePath, frontmatter) {
  ...
  const name = basename.replace(/\.(agent|prompt|instructions)\.md$/, "").replace(/\.md$/, "");
  return name
    .split("-")
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");
...
function generateAgentsData(gitDates) {
  ...
  for (const file of files) {
    ...
    agents.push({
      ...
      mcpServers: frontmatter?.["mcp-servers"] ? Object.keys(frontmatter["mcp-servers"]) : [],
      path: relativePath,
      filename: file,
...
function categorizeSkill(name, description) {
  ...
  if (text.includes("azure") || text.includes("appinsights")) {
    return "Azure";
  }
  if (
  ...
  if (text.includes("test") || text.includes("qa") || text.includes("playwright")) {
    return "Testing";
  }
  if (text.includes("microsoft") || text.includes("m365") || text.includes("workiq")) {
    return "Microsoft";
  }
  if (text.includes("cli") || text.includes("command")) {
    return "CLI Tools";
  }
  if (text.includes("diagram") || text.includes("plantuml") || text.includes("visual")) {
    return "Diagrams";
  }
  if (text.includes("nuget") || text.includes("dotnet") || text.includes(".net")) {
    return ".NET";
  }
...
function generateCollectionsData(gitDates) {
  ...
  const files = fs.readdirSync(COLLECTIONS_DIR).filter((f) => f.endsWith(".collection.yml"));
...
async function main() {
  ...
  // Write JSON files
  fs.writeFileSync(path.join(WEBSITE_DATA_DIR, "agents.json"), JSON.stringify(agentsData, null, 2));
  ...
  fs.writeFileSync(path.join(WEBSITE_DATA_DIR, "skills.json"), JSON.stringify(skillsData, null, 2));
  ...
  fs.writeFileSync(path.join(WEBSITE_DATA_DIR, "tools.json"), JSON.stringify(toolsData, null, 2));
  ...
  fs.writeFileSync(path.join(WEBSITE_DATA_DIR, "manifest.json"), JSON.stringify(manifest, null, 2));
