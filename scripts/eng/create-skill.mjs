...
async function createSkillTemplate() {
  try {
    ...
    if (!description) {
      description = await prompt("Description (what this skill does and when to use it): ");
    }

    if (!description || description.trim().length < 10) {
      console.error("❌ Description is required and must be at least 10 characters (max 1024)");
      process.exit(1);
    }
...
`;
    ...
    if (addAssets.toLowerCase() === "y" || addAssets.toLowerCase() === "yes") {
      ...
      console.log("   Common bundled assets: helper scripts, code templates, reference data");
    ...
    console.log("\n📖 Resources:");
    console.log("   - Anthropic Skills Spec: https://agentskills.io/specification");
    console.log("   - Project Documentation: AGENTS.md (section on Agent Skills)");
  } catch (error) {
