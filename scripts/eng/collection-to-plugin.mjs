...
function generateReadme(collection, items) {
  ...
  if (prompts.length > 0) {
    ...
    for (const item of prompts) {
      ...
      const description = item.frontmatter?.description || item.frontmatter?.title || name;
  ...
  if (agents.length > 0) {
    ...
    for (const item of agents) {
      ...
      const description = item.frontmatter?.description || item.frontmatter?.name || name;
 ...
 */
function convertCollectionToPlugin(collectionId, mode = "migrate", silent = false) {
  const collectionFile = path.join(COLLECTIONS_DIR, `${collectionId}.collection.yml`);
  ...
  if (mode === "migrate") {
    ...
    if (pluginExists) {
      ...
      console.log("💡 Use 'npm run plugin:refresh' to update an existing plugin.");
      process.exit(1);
    }
    ...
    if (!pluginExists) {
      ...
      console.log("💡 Use 'npm run plugin:migrate' to create a new plugin first.");
      process.exit(1);
    }
  ...
  for (const item of collection.items || []) {
    ...
    switch (item.kind) {
      case "prompt":
        ...
        const promptName = path.basename(item.path).replace(".prompt.md", ".md");
      ...
      case "skill":
        ...
        if (!fs.statSync(skillSrcDir).isDirectory()) {
          console.warn(`⚠️  Skill path is not a directory, skipping: ${item.path}`);
          continue;
        }
      ...
      default:
        console.warn(`⚠️  Unknown item kind "${item.kind}", skipping: ${item.path}`);
        continue;
    }
  ...
  console.log("\n📁 Generated files:");
  console.log(`   - ${path.join(pluginDir, ".github", "plugin", "plugin.json")}`);
...
async function main() {
  try {
    ...
    if (!collectionId) {
      ...
      collectionId = await prompt("Enter collection ID (or number from list): ");
