...
export const TYPE_PATTERNS = {
  ...
  infra: [".github/workflows/**/*.yml", ".github/workflows/**/*.yaml", "**/*.yml", "**/*.yaml"],
...
export const getMissingContributors = () => {
  try {
    ...
    console.log(
      `📋 Found ${filteredUsernames.length} missing contributors after filtering: ${filteredUsernames.join(", ")}`
    );
    ...
    if (details.toLowerCase().includes("bad credentials") || details.includes("401")) {
      ...
      console.error(
        "💡 Set a valid token in PRIVATE_TOKEN (all-contributors-cli) and/or GH_TOKEN (gh CLI)."
      );
    ...
    if (
      String(error?.message ?? "").includes("command not found") ||
      String(error?.message ?? "").includes("not recognized")
    ) {
      console.error(
        "💡 Make sure all-contributors-cli is installed: npm install all-contributors-cli"
      );
    }
...
export const fetchContributorMergedPrs = (username, { includeAllFiles = false } = {}) => {
  try {
    ...
    return prs.filter((pr) => {
      const hasNonConfigFiles = pr.files.some((file) => !isAutoGeneratedFile(file.path));
...
export const generateMarkdownReport = (reports, missingCount = 0) => {
  ...
  for (const report of reports) {
    ...
    for (const pr of prs) {
      ...
      lines.push(`[#${pr.prNumber}](${url}) ${title}`, "```plaintext", comment, "```");
    }
