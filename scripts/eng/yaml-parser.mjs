...
function extractMcpServerConfigs(filePath) {
  ...
  return Object.entries(metadata.mcpServers).map(([name, cfg]) => {
    ...
    return {
      ...
      headers: typeof copy.headers === "object" && copy.headers !== null ? copy.headers : undefined,
    };
  });
}
  ...
  return safeFileOperation(
    () => {
      ...
      if (!frontmatter?.name || !frontmatter?.description) {
        console.warn(`Invalid skill at ${skillPath}: missing name or description in frontmatter`);
        return null;
      }
