...
import { COLLECTIONS_DIR, MAX_COLLECTION_ITEMS, ROOT_FOLDER } from "./constants.mjs";
...
function validateAgentFile(filePath) {
  try {
    ...
    if (agent["mcp-servers"]) {
      if (typeof agent["mcp-servers"] !== "object" || Array.isArray(agent["mcp-servers"])) {
      ...
      for (const [serverName, serverConfig] of Object.entries(agent["mcp-servers"])) {
        ...
        if (serverConfig.tools !== undefined && !Array.isArray(serverConfig.tools)) {
...
function validateCollectionItems(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!["prompt", "instruction", "agent", "skill"].includes(item.kind)) {
      return `Item ${i + 1} kind must be one of: prompt, instruction, agent, skill`;
    }
