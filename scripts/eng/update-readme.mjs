    ...
    do {
      ...
      for (const entry of servers) {
        ...
        if (serverName) {
          ...
          const displayName =
            entry?.server?._meta?.["io.modelcontextprotocol.registry/publisher-provided"]?.github
...
function generateInstructionsSection(instructionsDir) {
  ...
  // Create table header
  let instructionsContent = "| Title | Description |\n| ----- | ----------- |\n";
...
function generatePromptsSection(promptsDir) {
  ...
  const promptFiles = fs.readdirSync(promptsDir).filter((file) => file.endsWith(".prompt.md"));
...
function generateSkillsSection(skillsDir) {
  ...
  for (const skill of skillEntries) {
    ...
    const assetsList =
      skill.assets.length > 0 ? skill.assets.map((a) => `\`${a}\``).join("<br />") : "None";
...
function generateUnifiedModeSection(cfg) {
  ...
  console.log(`Unified mode generator: ${entries.length} files for extension ${extension}`);
  if (entries.length === 0) {
    return "";
  }
  ...
  for (const { file, filePath, title } of entries) {
    ...
    const descCell = description && description !== "null" ? formatTableCell(description) : "";
    if (includeMcpServers) {
...
function generateCollectionsSection(collectionsDir) {
  ...
  const featuredCollections = collectionEntries.filter((entry) => entry.isFeatured);
  const regularCollections = collectionEntries.filter((entry) => !entry.isFeatured);
  ...
  for (const entry of sortedEntries) {
    ...
    const description = formatTableCell(collection.description || "No description");
    ...
    .map((file) => {
      ...
      return safeFileOperation(
        () => {
          ...
          if (!collection.display?.featured) {
            return null;
          }
...
function buildCollectionRow({
  ...
  const titleCell = badges ? `[${title}](${link})<br />${badges}` : `[${title}](${link})`;
  ...
  if (hasAgents) {
    ...
    const mcpServers = kind === "agent" ? extractMcpServerConfigs(filePath) : [];
    const mcpServerCell =
...
async function main() {
  try {
    ...
    writeFileIfChanged(path.join(DOCS_DIR, "README.instructions.md"), instructionsReadme);
    ...
    writeFileIfChanged(path.join(DOCS_DIR, "README.collections.md"), collectionsReadme);
    ...
    if (fs.existsSync(COLLECTIONS_DIR)) {
      ...
      for (const file of collectionFiles) {
        ...
        if (collection) {
          const collectionId = collection.id || path.basename(file, ".collection.yml");
          const readmeContent = generateCollectionReadme(collection, collectionId, registryNames);
