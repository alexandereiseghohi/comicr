...
├── scripts/eng/              # Build and automation scripts
...
3. Update the README.md by running: `pnpm run build`
4. Verify the resource appears in the generated README

**For Skills:**

1. Run `pnpm run skill:create` to scaffold a new skill folder
...
4. Run `pnpm run skill:validate` to validate the skill structure
5. Update the README.md by running: `pnpm run build`
...
- Run `pnpm run build` to update the README
...
- Located in `scripts/eng/` and `scripts/` directories
...
1. **README updates**: New files should automatically be added to the README when you run `pnpm run build`
...
4. **Build check**: Run `pnpm run build` before committing to verify README generation
...
7. **Testing**: If adding a collection, run `pnpm run collection:validate` to ensure validity

### Pre-commit Checklist

Before submitting your PR, ensure you have:

- [ ] Run `pnpm install` (or `pnpm ci`) to install dependencies
- [ ] Run `pnpm run build` to generate the updated README.md
