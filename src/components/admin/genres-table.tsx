...
export function GenresTable({ items }: GenresTableProps) {
  ...
  const columns: ColumnDef<GenreRow>[] = [
    ...
    { accessorKey: "name", header: "Name" },
    {
      ...
      header: "Slug",
      cell: ({ row }) => (
        <code className="text-xs bg-muted px-1 py-0.5 rounded">{row.original.slug}</code>
      ),
    },
    {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link href={`/admin/genres/${row.original.id}/edit`}>
            ...
            {row.original.isActive ? (
              <DropdownMenuItem
                onClick={async () => {
                  await deleteGenreAction(row.original.id);
                  router.refresh();
                }}
              >
              ...
              <DropdownMenuItem
                onClick={async () => {
                  await restoreGenreAction(row.original.id);
                  router.refresh();
                }}
              >
  ...
  const handleBulkDelete = async () => {
    await bulkDeleteGenresAction(selectedIds);
    setSelectedIds([]);
    setRowSelection({});
    router.refresh();
  };
  ...
  const handleBulkRestore = async () => {
    await bulkRestoreGenresAction(selectedIds);
    setSelectedIds([]);
    setRowSelection({});
    router.refresh();
  };
